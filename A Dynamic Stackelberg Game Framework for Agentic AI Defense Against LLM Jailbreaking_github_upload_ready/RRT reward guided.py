...
def plot_jailbreak_prompts(jailbreak_prompts):
    ...
    plt.tight_layout()
    plt.savefig("purple_agent_tsne_ours.png", dpi=300)
    plt.show()
...
def rollout_trim(x0, T, N, M, history):
    ...
    for i in range(N):
        ...
        for t in range(T):
            ...
            if result == "Jailbreak":
                ...
                #tau.append(p_new)  # do not add new node to the trajectory-it is the outcome node
            ...
            if count_LLM_API % 10 == 0:
                print("count_LLM_API=", count_LLM_API,"count_jailbreaking=", count_jailbreaking)
    ...
    for tau, reward in top_M:
        if len(tau) > 1:
            ...
            rollout_trim(x1, T, N, M, history)  # Recursive call for the next rollout
        else:
...
if __name__ == "__main__":
    ...
    top_M = rollout_trim(DEFAULT_ROOT_PROMPT, T, N, M,history)  # Starting node, depth T, 20 rollouts, top 5
    print("top_M:", top_M)
